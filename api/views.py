...
def zabbix_memory_get(request):
    if request.method=="POST":
        ...
        for item in data['host_item_ids']:
            memory_total=cpu_data[data['host_item_ids'][item]['vm.memory.size[total]']][0][1]
            ...
            for i in range(len(cpu_data[data['host_item_ids'][item]['vm.memory.size[available]']])):
                ...
                # cpu_data[data['host_item_ids'][item]['vm.memory.size[available]']][i].append(round(round(memory_total-memory_available,2)/memory_total,2)*100)
